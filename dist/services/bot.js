...
var commandHandlers_1 = require("./commandHandlers");
...
exports.initializeBot = function () {
    ...
    bot.command("subscribe", function (ctx) { exports.subscriberId.push(ctx.chat.id); ctx.reply("subscribed successfully with other " + exports.subscriberId.length + "users"); });
    ...
    bot.command("unsubscribe", function (ctx) {
        exports.subscriberId = exports.subscriberId.filter(function (id) { return id !== ctx.chat.id; });
        ctx.reply("Unsubscribed from all updates.");
    });
    // bot.command("rsi", getRSIStatus)
    // bot.command('subscribeob', overBoughtSignal)
    bot.command("ping", function (ctx) { return ctx.reply("Got another message!" + ctx.chat.id); });
    bot.start();
